...
class SimpleController(app_manager.RyuApp):
    ...
    def switch_features_handler(self, ev):
        ...
        self.add_empty_flow(datapath, ofproto, parser)
    ...
    def delete_previous_flows(self):
        for switch in self.switches.values():
            datapath = switch.dp
            ofproto = datapath.ofproto
            parser = datapath.ofproto_parser

            # Match all flows (wildcard match)
            match = parser.OFPMatch()  # Empty match to match all flows

            # Create a FlowMod message to delete all flows
            flow_mod = parser.OFPFlowMod(
                datapath=datapath,
                command=ofproto.OFPFC_DELETE,  # Delete command
                out_port=ofproto.OFPP_ANY,     # Match any outgoing port
                out_group=ofproto.OFPG_ANY,    # Match any group
                priority=1,                    # Start from priority 1 and higher
                match=match
            )

            # Send the FlowMod message to the switch
            datapath.send_msg(flow_mod)
            self.logger.info(f"Deleted all flows on switch {datapath.id}")
            
            self.add_empty_flow(datapath, ofproto, parser)

    def add_empty_flow(self, datapath, ofproto, parser):
        self.logger.info("Adding empty flow to switch %d", datapath.id)
